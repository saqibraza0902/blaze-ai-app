...
export const pickSingleFile = async (
  ...
  try {
    ...
    if (
      ...
      Toast.show(`File is larger than ${options.maxSizeMB}MB`, 500);
      return null;
    }
    ...
    console.log('File copied successfully:', localCopy);

    if (localCopy.status === 'success') {
      const decodedUri = decodeURI(localCopy.localUri); // 🔥 Fix: Decode URI here

      return {
        uri: decodedUri, // Use decoded URI
        name: res.name ?? 'unnamed',
        type: res.type ?? null,
        size: res.size ?? null,
        fileCopyUri: decodedUri,
      };
    } else {
